...
void HTAnalyzer::WorkerThread()
{
	...
	for( ; ; )
	{
		...
		mMosiSerial->Advance( samples_to_first_center_of_first_data_bit );

		for( U32 i=0; i<8; i++ )
		{
			//let's put a dot exactly where we sample this bit:
			mResults->AddMarker( mMosiSerial->GetSampleNumber(), AnalyzerResults::Dot, mSettings->mMosiChannel );

			if( mMosiSerial->GetBitState() == BIT_HIGH )
				data |= mask;

			mMosiSerial->Advance( samples_per_bit );
